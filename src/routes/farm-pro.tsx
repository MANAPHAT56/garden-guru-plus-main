...
function FarmProPage() {
  ...
  return (
    <AppShell title="งานและทีม" subtitle="มอบหมายงาน · ติดตามคน · วางแผนผลิต">
      ...
      {showTaskForm ? <Card className="space-y-2 border-primary/25 bg-primary-soft/35"><p className="text-xs font-semibold text-primary">สร้าง Task จากแผนงานทีม</p><input value={newTaskTitle} onChange={(event) => setNewTaskTitle(event.target.value)} placeholder="ชื่องานที่พนักงานต้องทำ" className="w-full rounded-lg border border-border bg-card px-3 py-2 text-sm outline-none focus:border-primary" /><div className="grid grid-cols-2 gap-2"><select value={newTaskPlot} onChange={(event) => setNewTaskPlot(event.target.value)} className="rounded-lg border border-border bg-card px-2 py-2 text-xs"><option value="">เลือกแปลง</option>{plots.filter((plot) => plot !== "ทั้งหมด").map((plot) => <option key={plot}>{plot}</option>)}</select><select value={newTaskTeam} onChange={(event) => setNewTaskTeam(event.target.value)} className="rounded-lg border border-border bg-card px-2 py-2 text-xs"><option value="">เลือกทีม</option>{teams.filter((team) => team !== "ทั้งหมด").map((team) => <option key={team}>{team}</option>)}</select></div><select value={newTaskWorkerId} onChange={(event) => setNewTaskWorkerId(event.target.value)} className="w-full rounded-lg border border-border bg-card px-2 py-2 text-xs"><option value="">มอบหมายภายหลัง</option>{state.workers.filter((worker) => !newTaskTeam || worker.crew === newTaskTeam).map((worker) => <option key={worker.id} value={worker.id}>{worker.name} · {worker.crew}</option>)}</select><button onClick={() => { if (!newTaskTitle.trim() || !newTaskPlot || !newTaskTeam) { setActionMessage("กรอกชื่องาน แปลง และทีมก่อนมอบหมาย"); return; } addTask({ title: newTaskTitle, plot: newTaskPlot, team: newTaskTeam, assignedWorkerId: newTaskWorkerId || undefined, type: "General", origin: "team", createdBy: persona.role, status: newTaskWorkerId ? "Assigned" : "Planned", scheduledFor: new Date().toISOString().slice(0, 10) }); setShowTaskForm(false); setNewTaskTitle(""); setActionMessage("สร้างงานทีมแล้ว งานจะปรากฏใน งานของฉัน ของผู้รับผิดชอบ"); }} className="w-full rounded-lg bg-primary py-2 text-xs font-semibold text-primary-foreground">สร้างและมอบหมายงาน</button></Card> : null}
      <div className="space-y-3">
        {workOrders.map((wo) => (
          <Card key={wo.id}>

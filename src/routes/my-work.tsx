...
function MyWorkPage() {
  const { state, persona, updateTaskStatus } = useDragonflyData();
  ...
  const taskList = useMemo(() => state.tasks.filter((task) => {
    const belongsToCurrentUser = isEmployee ? task.assignedWorkerId === assignee?.id : task.origin === "personal" && task.ownerPersonaId === persona.id;
  ...
  return (
    <AppShell title="งานของฉัน" subtitle={isEmployee ? `${assignee?.name ?? "พนักงาน"} · วันนี้ · มุมมองพนักงานภาคสนาม` : `${persona.role} · Todo งานส่วนตัว`}>
      {isEmployee ? <Card className="border-primary/25 bg-primary-soft/45">
      ...
      <div className="space-y-3">
        {taskList.map((task) => <Card key={task.id}>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            {task.status === "Supervisor Review" ? <span className="flex items-center justify-center gap-1 rounded-lg bg-primary-soft py-2 text-xs font-semibold text-primary">รอตรวจรับ</span> : task.status !== "Completed" ? <button onClick={() => { if (task.status === "In Progress") { setCompletingTaskId(task.id); setCompletionNote(""); setCompletionHealth(""); setEvidenceCount(0); setLaborHours(""); setLaborCost(""); setMaterialCost(""); } else { updateTaskStatus(task.id, "In Progress"); setMessage("เริ่มบันทึกเวลาปฏิบัติงานแล้ว"); } }} className="rounded-lg bg-primary py-2 text-xs font-semibold text-primary-foreground">{task.status === "In Progress" ? "ส่งตรวจ" : "เริ่มงาน"}</button> : <span className="flex items-center justify-center gap-1 rounded-lg bg-primary-soft py-2 text-xs font-semibold text-primary"><CheckCircle2 className="size-3.5" />อนุมัติแล้ว</span>}
